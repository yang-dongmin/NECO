...
import { BookOpen, LayoutDashboard, RotateCcw, BarChart2, Plus, Star, Globe2, Code2, CalendarCheck } from 'lucide-react'
import { useAuthStore, useNoteStore } from '../store'
import { useSrsStore } from '../store/srsStore'
import { fetchDueCodeNotes, fetchStreak } from '../api/client'
import { SUBJECTS } from '../api/mock'
import logo from '../assets/neco.png'

const NAV = [
  { to:'/',                  icon:LayoutDashboard, label:'대시보드'       },
  { to:'/review-dashboard',  icon:CalendarCheck,   label:'복습 대시보드', combinedDue:true },
...
export default function Sidebar() {
  ...
  const [codeDue, setCodeDue] = useState(0)
  useEffect(() => {
    const load = () => fetchDueCodeNotes().then(d => setCodeDue(d?.due?.length ?? 0)).catch(() => {})
    load()
  ...
  useEffect(() => {
    if (!user) return
    fetchStreak().then(d => setStreak(d?.currentStreak ?? 0)).catch(() => {})
  }, [user])

  const combinedDue    = due + codeDue
  ...
  useEffect(() => {
    const check = async () => {
      try {
        const res = await fetch('http://localhost:3939/api/status', { signal: AbortSignal.timeout(1500) })
  ...
  return (
    <aside style={{ width:232, minWidth:232, background:'#fff', borderRight:'1px solid #f1f5f9', display:'flex', flexDirection:'column', height:'100vh' }}>
      ...
      <div style={{ padding:'20px 20px 16px', borderBottom:'1px solid #f1f5f9' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9 }}>
          <div style={{ width:32, height:32, borderRadius:8, background:'linear-gradient(135deg,#2563eb,#10b981)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            <img src={logo} alt="logo" style={{ width:60, height:60, borderRadius:8, objectFit:'contain' }} />
          </div>
          <div>
      ...
      <nav style={{ padding:'12px 12px 0', flex:1, overflowY:'auto' }}>
        <SLabel>메뉴</SLabel>
        {NAV.map(({ to, icon:Icon, label, srs, combinedDue: showCombined }) => (
          <NavLink key={to} to={to} end={to==='/'} style={{ textDecoration:'none' }}>
            {({ isActive }) => (
              <div
                style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 10px', borderRadius:8, marginBottom:2, cursor:'pointer', background:isActive?'#eff6ff':'transparent', color:isActive?'#2563eb':'#475569', fontWeight:isActive?600:400, fontSize:13, transition:'all 0.12s' }}
              ...
              >
                ...
                {showCombined && combinedDue>0 && (
                  <span style={{ fontSize:10, fontWeight:700, padding:'1px 7px', borderRadius:99, background:'#fef3c7', color:'#d97706', border:'1px solid #fde68a' }}>{combinedDue}</span>
                )}
              </div>
            )}
          </NavLink>
        ...
        <div
          ...
          style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 10px', borderRadius:8, marginBottom:2, cursor:'pointer', fontSize:13, transition:'all 0.12s', background:isBookmarkActive?'#eff6ff':'transparent', color:isBookmarkActive?'#2563eb':'#475569', fontWeight:isBookmarkActive?600:400 }}
        ...
        {SUBJECTS.map(s => {
          ...
          return (
            <div
              ...
              style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 10px', borderRadius:8, cursor:'pointer', fontSize:12, marginBottom:1, transition:'all 0.12s', background:isActive?`${s.color}12`:'transparent', color:isActive?s.color:'#475569' }}
              onMouseEnter={e=>{ if(!isActive) e.currentTarget.style.background='#f8fafc' }}
              onMouseLeave={e=>{ if(!isActive) e.currentTarget.style.background='transparent' }}
            >
              ...
              {count > 0 && (
                <span style={{ fontSize:9, padding:'1px 6px', borderRadius:99, background:isActive?s.color:'#f1f5f9', color:isActive?'#fff':'#94a3b8' }}>{count}</span>
              )}
            </div>
          )
        })}
        ...
        {combinedDue > 0 && (
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:'#64748b' }}>{combinedDue}개 대기 중</div>
            </div>
          </div>
        )}
      </nav>
      ...
      <div style={{ padding:12, borderTop:'1px solid #f1f5f9' }}>
        ...
        {/* VSCode 연결 상태 */}
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 10px', borderRadius:8, marginBottom:8, background:vsConnected?'#f0fdf4':'#f8fafc', border:`1px solid ${vsConnected?'#bbf7d0':'#e2e8f0'}` }}>
          <div style={{ width:7, height:7, borderRadius:'50%', flexShrink:0, background:vsConnected?'#10b981':'#cbd5e1', boxShadow:vsConnected?'0 0 0 2px #d1fae5':'none' }} />
          <div style={{ flex:1 }}>
            <div style={{ fontSize:11, fontWeight:600, color:vsConnected?'#059669':'#94a3b8' }}>VSCode {vsConnected ? '연결됨' : '연결 안 됨'}</div>
            <div style={{ fontSize:9, color:'#94a3b8' }}>{vsConnected ? 'NECO 확장 실행 중' : 'VSCode에서 NECO 열기'}</div>
          </div>
        </div>
        ...
        {/* 스트릭 */}
        {user && streak > 0 && (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'7px 10px', borderRadius:8, marginBottom:8, background:'linear-gradient(135deg,#fff7ed,#fef3c7)', border:'1px solid #fde68a' }}>
            <span style={{ fontSize:16 }}>🔥</span>
            <div style={{ fontSize:12, fontWeight:700, color:'#d97706' }}>{streak}일 연속 복습 중!</div>
          </div>
        )}
