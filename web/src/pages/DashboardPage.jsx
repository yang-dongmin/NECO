...
import { SUBJECTS, getDday } from '../api/mock'   // getDday·SUBJECTS 상수만 유지
...
export default function DashboardPage() {
  const { setStats, setNotes } = useNoteStore()
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      try {
        // 노트 목록 (최근 50개 — 대시보드 위젯용)
        const [notesData, statsData] = await Promise.all([
          getNotes({ limit: 50, sort: 'newest' }),
          getStats(),
        ])
        const notes = notesData.notes ?? []
        setAllNotes(notes)
        setNotes(notes, notesData.pagination)
  ...
  const srsSummary = useMemo(() => getSummary(allNotes),        [allNotes])
  const enriched   = useMemo(() => getEnrichedCards(allNotes),  [allNotes])
  const dueCards   = useMemo(() => enriched.filter(n => n.srs.due).slice(0, 3), [enriched])
  const dday       = getDday()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          {dueCards.length === 0 ? (
            ...
            <>
              {dueCards.map(n => {
                ...
                return (
                  <div key={n.id} onClick={() => navigate(`/notes/${n.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}>
        ...
        {/* 과목별 문제 현황 — API subjectBreakdown 우선, 없으면 allNotes 기반 */}
        <Panel title="과목별 문제 현황">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {SUBJECTS.map(s => {
              // API stats의 subjectBreakdown 우선 사용
              const apiCount = apiStats?.subjectBreakdown?.find(b => b.subject === s.id)?.count ?? 0
              const count = apiCount || allNotes.filter(n => n.subject === s.id).length
              ...
              return (
                <div key={s.id}>
                  ...
                  <div style={{ height: 6, background: '#f1f5f9', borderRadius: 99, overflow: 'hidden' }}>
                    <div style={{ width: `${pct}%`, height: '100%', background: s.color, borderRadius: 99, transition: 'width 0.5s ease' }} />
                  </div>
                </div>
              )
            })}
