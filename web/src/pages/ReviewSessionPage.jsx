...
export default function ReviewSessionPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const params = { limit: 200, sort: 'newest' }
        if (subjectFilter) params.subject = subjectFilter
        const data = await getNotes(params)
        const all  = data.notes ?? []
        const due  = getDueNotes(all)
        setQueue(due.length > 0 ? due : all)
      } catch (err) {
        console.error('[ReviewSession] 노트 로드 실패:', err)
        setQueue([])
      }
    }
    load()
  ...
  const handleQuality = useCallback((quality) => {
    ...
    const next = [...results, {
      noteId: current.id,
      noteTitle: (current.wrongCode || current.code || '문제').split('\n')[0].slice(0, 40),
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div style={{ flexShrink:0, textAlign:'right' }}>
            <div style={{ fontSize:12, fontWeight:700, color:'#1e293b', fontFamily:'JetBrains Mono, monospace' }}>
              {idx+1} / {queue.length}
            </div>
            <div style={{ fontSize:10, color:'#94a3b8' }}>
              {Math.round((idx / queue.length) * 100)}% 완료
            </div>
          </div>
        </div>
