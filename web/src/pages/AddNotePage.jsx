...
import { createNote, getNote, updateNote, getTags } from '../api/client'
    ...
    <div style={{ position:'relative' }}>
      <textarea value={value} onChange={e=>onChange(e.target.value)}
        ...
        style={{
          ...
          background:'#f8fafc', borderRadius:8, padding:'12px 14px', paddingBottom:26,
          fontFamily:'JetBrains Mono,monospace', fontSize:12.5, lineHeight:1.8,
          color:'#1e293b', outline:'none', boxSizing:'border-box',
          border:`1.5px solid ${focused?(accent??'#2563eb'):'#e2e8f0'}`,
          boxShadow: focused?`0 0 0 3px ${accent?accent+'22':'rgba(37,99,235,0.1)'}`:undefined,
...
function TagInput({ selected, onChange, suggestions: allSuggestions = [] }) {
  const [input, setInput] = useState('')
  const suggestions = allSuggestions.filter(n=>!selected.includes(n))
...
function QuizBuilder({ onInsert }) {
  ...
  const updateChoice = (i, val) => {
    const next = [...choices]; next[i] = val; setChoices(next)
  }
  const handleInsert = () => {
    ...
    const body = question + '\n\n' + choices
      ...
      .join('\n')
    onInsert(body)
    setQuestion(''); setChoices(['', '', '', '']); setOpen(false)
  }
  ...
  return (
    <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:9, padding:14, marginBottom:10 }}>
      ...
      <div style={{ display:'flex', gap:8, marginTop:8 }}>
        <button onClick={handleInsert} style={{ flex:1, padding:'7px 0', borderRadius:7, background:'#10b981', border:'none', color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer' }}>문제에 삽입</button>
        <button onClick={()=>setOpen(false)} style={{ padding:'7px 14px', borderRadius:7, background:'transparent', border:'1px solid #bbf7d0', color:'#64748b', fontSize:12, cursor:'pointer' }}>닫기</button>
      </div>
    </div>
  )
}
...
export default function AddNotePage() {
  ...
  const { id }     = useParams()
  const isEdit     = !!id
  const { addNote, updateNote: storeUpdateNote } = useNoteStore()
  const toast      = useToast()

  const EMPTY = { subject:'sw-design', year:new Date().getFullYear(), round:1, wrongCode:'', fixedCode:'', explanation:'', language:'theory', tags:[] }
  const [apiTagNames, setApiTagNames] = useState([])
  useEffect(() => {
    getTags().then(data => setApiTagNames((data.tags ?? []).map(t => t.name))).catch(() => {})
  }, [])
    ...
    if (isEdit) {
      getNote(id)
        .then(data => {
          ...
          setForm({
            subject:     note.subject     || 'programming',
            year:        note.year        || new Date().getFullYear(),
            round:       note.round       || 1,
            language:    note.language    || 'theory',
            wrongCode:   note.wrongCode   || '',
            fixedCode:   note.fixedCode   || '',
            explanation: note.explanation || '',
            tags:        (note.tags || []).map(t => t.name || t),
          })
        })
        .catch(() => { toast({ message:'노트를 불러오지 못했습니다.', type:'error' }); navigate('/notes') })
    } else {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      wrongCode:   form.wrongCode.trim(),
      fixedCode:   form.fixedCode.trim(),
    ...
    try {
      if (isEdit) {
        ...
        const updated = data.note ?? { ...form, id: Number(id) }
        storeUpdateNote(Number(id), {
          ...updated,
          tags: (updated.tags || form.tags.map((name,i)=>({id:i,name}))),
        })
  ...
  return (
    <div style={{ maxWidth:860, margin:'0 auto', display:'flex', flexDirection:'column', gap:16 }}>
          ...
          {showPreview ? (
            ...
            <div style={{ position:'relative' }}>
              <textarea value={form.explanation} onChange={e=>set('explanation',e.target.value)}
                ...
                style={{ width:'100%', minHeight:110, resize:'vertical', padding:'12px 14px', paddingBottom:28, borderRadius:8, border:'1px solid #e2e8f0', background:'#fff', fontSize:13, lineHeight:1.8, color:'#1e293b', outline:'none', boxSizing:'border-box' }}
                onFocus={e=>e.target.style.borderColor='#2563eb'}
              ...
              <div style={{
                ...
                color: form.explanation.length > 1000 ? '#ef4444' : form.explanation.length > 500 ? '#f59e0b' : '#94a3b8',
                pointerEvents:'none',
              }}>
                {form.explanation.length} 자
                {form.explanation.length > 1000 && ' · 너무 길어요'}
              </div>
            </div>
          )}
        </Field>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        ...
        <div style={{ background:'#fff', border:'1px solid #f1f5f9', borderRadius:12, padding:'18px 20px', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
          <Field label="태그">
            <TagInput selected={form.tags} onChange={tags=>set('tags',tags)} suggestions={apiTagNames} />
          </Field>
        </div>
      </div>
