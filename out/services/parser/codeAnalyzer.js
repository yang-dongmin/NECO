...
function extractCalledFunctions(node, languageId, result = new Set()) {
...
*/
function findFunctionNode(node, languageId) {
...
*/
function extractParams(paramsNode, languageId) {
    const params = [];
    for (const child of paramsNode.children) {
        ...
        if (['typescript', 'typescriptreact', 'javascript', 'javascriptreact'].includes(languageId)) {
            if (child.type === 'required_parameter' ||
                ...
                const nameNode = child.children.find((c) => c.type === 'identifier' || c.type === 'rest_pattern');
                const typeNode = child.children.find((c) => c.type === 'type_annotation');
                params.push({
                    name: nameNode?.text ?? child.text,
                    type: typeNode?.children.find((c) => c.type !== ':')?.text,
                });
                continue;
            }
        ...
        if (['java', 'csharp'].includes(languageId)) {
            if (child.type === 'formal_parameter') {
                const typeNode = child.children.find((c) => c.type === 'type_identifier' || c.type === 'integral_type' || c.type === 'generic_type');
                const nameNode = child.children.find((c) => c.type === 'identifier');
                params.push({
                    name: nameNode?.text ?? child.text,
                    type: typeNode?.text,
        ...
        if (languageId === 'rust') {
            if (child.type === 'parameter') {
                const nameNode = child.children.find((c) => c.type === 'identifier' || c.type === 'pattern');
                const typeNode = child.children.find((c) => c.type !== 'identifier' && c.type !== ':' && c.type !== 'pattern');
                params.push({
                    name: nameNode?.text ?? child.text,
                    type: typeNode?.text,
                });
...
/*
  ...
  - 없으면 undefined
*/
function extractReturnType(funcNode, languageId) {
    if (['typescript', 'typescriptreact', 'javascript', 'javascriptreact'].includes(languageId)) {
        const returnTypeNode = funcNode.children.find((c) => c.type === 'type_annotation');
        return returnTypeNode?.children.find((c) => c.type !== ':')?.text;
    }
    if (['java', 'csharp'].includes(languageId)) {
        const typeNode = funcNode.children.find((c) => c.type === 'type_identifier' || c.type === 'integral_type' || c.type === 'void_type' || c.type === 'generic_type');
        return typeNode?.text;
    }
    if (languageId === 'go') {
        // Go는 파라미터 목록 뒤에 반환 타입이 온다
        const paramIdx = funcNode.children.findIndex((c) => c.type === 'parameter_list');
        if (paramIdx !== -1) {
    ...
    if (languageId === 'rust') {
        const returnNode = funcNode.children.find((c) => c.type === 'return_type');
        return returnNode?.children.find((c) => c.type !== '->')?.text;
    }
    return undefined;
}
/*
...
*/
async function analyzeCode(code, languageId, extensionUri) {
    // 지원하지 않는 언어면 바로 실패
    if (!FUNCTION_NODE_TYPES[languageId]) {
        return { success: false, reason: `지원하지 않는 언어예요: ${languageId}` };
    }
    // AST 생성
    const tree = await (0, treeParser_1.parseCode)(code, languageId, extensionUri);
    if (!tree) {
    ...
    let params = [];
    if (['c', 'cpp'].includes(languageId)) {
        const declarator = funcNode.children.find((c) => c.type === 'function_declarator');
        if (declarator) {
            functionName = declarator.children.find((c) => c.type === 'identifier')?.text;
            const paramsNode = declarator.children.find((c) => c.type === 'parameter_list');
            if (paramsNode)
                params = extractParams(paramsNode, languageId);
        }
    }
    else {
        const nameNode = funcNode.children.find((c) => c.type === (NAME_NODE_TYPES[languageId] ?? 'identifier'));
        functionName = nameNode?.text;
        const paramsNodeType = PARAMS_NODE_TYPES[languageId] ?? 'formal_parameters';
        const paramsNode = funcNode.children.find((c) => c.type === paramsNodeType);
        params = paramsNode ? extractParams(paramsNode, languageId) : [];
    }
    // 반환 타입
    const returnType = extractReturnType(funcNode, languageId);
    // 핵심 로직
    const bodyNode = funcNode.children.find((c) => c.type === 'block' || c.type === 'statement_block' || c.type === 'body' || c.type === 'compound_statement');
    // 바디 노드에서 호출 함수 추출
